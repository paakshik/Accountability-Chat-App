...
import type { Event, Goal, GoalInput, LadderTier } from '@workspace/api-client-react';
import {
  AlertTriangle,
  ArrowRight,
  Check,
...
const apiError = (error: unknown) => {
  ...
  // `customFetch` throws an ApiError that carries the parsed JSON body on `.data`.
...
// A goal is editable while proposed, or once its 24-hour amendment window has elapsed.
// Mirrors `canChangeGoal` on the server so the UI does not offer a doomed request.
const canEditGoal = (goal: Goal) =>
  goal.status === 'proposed' || Boolean(goal.editUnlockAt && new Date(goal.editUnlockAt) <= new Date());
const canAmendGoal = (goal: Goal) => goal.status === 'active' || goal.status === 'locked';
...
type ChatTurn = { role: 'you' | 'partner'; text: string };

function ChatPanel() {
  const send = useSendChatMessage();
  const [message, setMessage] = useState('');
  // Kept in component state only. Per the product spec the partner's memory is the
  // stored record injected server-side each turn, not this transcript.
  const [turns, setTurns] = useState<ChatTurn[]>([]);
  const submit = (e: FormEvent) => {
    e.preventDefault();
    const text = message.trim();
    if (!text) return;
    // Send the transcript so far so follow-ups resolve; the server still treats stored
    // goal and event state as the authority on any fact.
    const history = turns.map(turn => ({ role: turn.role === 'you' ? 'user' as const : 'partner' as const, text: turn.text }));
    setTurns(current => [...current, { role: 'you', text }]);
    setMessage('');
    send.mutate({ data: { message: text, history } }, { onSuccess: result => setTurns(current => [...current, { role: 'partner', text: result.message }]) });
  };
  return <section data-testid="section-chat" className="soft-card overflow-hidden rounded-2xl border bg-card"><div className="flex items-center justify-between border-b px-5 py-4"><div><p className="flex items-center gap-2 text-sm font-bold"><MessageSquare size={16} className="text-primary" /> Ask the record</p><p className="mt-1 text-xs text-muted-foreground">A factual surface, not a coach.</p></div><span className="rounded-full bg-emerald-100 px-2 py-1 font-mono text-[10px] text-emerald-900">PRIVATE</span></div><div data-testid="chat-transcript" className="max-h-80 min-h-32 space-y-3 overflow-y-auto p-5">{turns.length ? turns.map((turn, index) => <div key={index} data-testid={`chat-turn-${index}`} className={turn.role === 'you' ? 'flex justify-end' : 'flex justify-start'}><div className={`max-w-[85%] whitespace-pre-wrap rounded-xl p-3 text-sm leading-relaxed ${turn.role === 'you' ? 'bg-primary text-primary-foreground' : 'bg-secondary'}`}><p className="mb-1 font-mono text-[10px] uppercase tracking-wider opacity-60">{turn.role === 'you' ? 'You' : 'Partner'}</p>{turn.text}</div></div>) : <div data-testid="chat-empty" className="flex min-h-24 flex-col justify-center"><p className="text-sm font-semibold">What do you need to verify?</p><p className="mt-1 text-xs text-muted-foreground">Try asking what you committed to, or when the next check-in is due.</p></div>}{send.isPending && <p data-testid="chat-pending" className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Checking the record...</p>}</div><form onSubmit={submit} className="flex gap-2 border-t p-3"><input data-testid="input-chat-message" value={message} onChange={e => setMessage(e.target.value)} placeholder="Ask about your record..." className="min-w-0 flex-1 rounded-xl border bg-background px-3 text-sm outline-none focus:border-primary" /><Button data-testid="button-send-chat" type="submit" disabled={send.isPending || !message.trim()} className="size-10 shrink-0 p-0" aria-label="Send message">{send.isPending ? <span className="size-3 animate-pulse rounded-full bg-primary-foreground" /> : <Send size={15} />}</Button></form>{send.isError && <p data-testid="status-chat-error" className="px-4 pb-4 text-xs text-red-800">{apiError(send.error)}</p>}</section>;
}

function SlipPanel() {
  const slip = useLogSlip();
  const ladder = useGetLadder();
  const qc = useQueryClient();
  const [minutes, setMinutes] = useState('');
  const [note, setNote] = useState('');
  const [result, setResult] = useState<{ minutes: number; tier: LadderTier | null } | null>(null);
  const parsed = Number(minutes);
  const valid = Number.isInteger(parsed) && parsed >= 1 && parsed <= 1440;
  // Show which rung will apply before submitting: highest threshold at or below the slip.
  const preview = valid ? [...(ladder.data?.tiers ?? [])].filter(t => t.slipMinutes <= parsed).at(-1) ?? null : null;
  const submit = (e: FormEvent) => {
    e.preventDefault();
    if (!valid) return;
    slip.mutate({ data: { minutes: parsed, note: note.trim() || undefined } }, { onSuccess: r => {
      setResult({ minutes: r.minutes, tier: r.tier ?? null });
      setMinutes(''); setNote('');
      qc.invalidateQueries({ queryKey: getGetDashboardQueryKey() });
      qc.invalidateQueries({ queryKey: getListEventsQueryKey({ limit: 8 }) });
    } });
  };
  return <section data-testid="section-slip" className="soft-card rounded-2xl border bg-card p-5"><div className="mb-3 flex items-start justify-between gap-3"><div><p className="flex items-center gap-2 text-sm font-bold"><Timer size={16} className="text-primary" /> Report a slip</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Time lost is a fact like any other. The consequence comes from your ladder, not from here.</p></div></div>
    <form onSubmit={submit} className="grid gap-2 sm:grid-cols-[7rem_1fr_auto]"><div className="flex items-center gap-1 rounded-xl border bg-background/60 px-2"><input data-testid="input-slip-minutes" type="number" min={1} max={1440} value={minutes} onChange={e => setMinutes(e.target.value)} placeholder="20" className="h-11 w-full bg-transparent text-sm outline-none" /><span className="font-mono text-[10px] text-muted-foreground">MIN</span></div><input data-testid="input-slip-note" value={note} onChange={e => setNote(e.target.value)} placeholder="What did it go to?" className="h-11 min-w-0 rounded-xl border bg-background/60 px-3 text-sm outline-none focus:border-primary" /><Button data-testid="button-log-slip" type="submit" disabled={!valid || slip.isPending}>{slip.isPending ? 'Recording...' : 'Record'}</Button></form>
    {preview && !result && <p data-testid="text-slip-preview" className="mt-3 rounded-xl bg-secondary p-3 text-xs leading-relaxed"><span className="font-mono text-[10px] uppercase tracking-wider opacity-60">Applies · {preview.slipMinutes} min rung</span><br />{preview.consequence}</p>}
    {result && <p data-testid="text-slip-result" className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-relaxed text-amber-950"><span className="font-mono text-[10px] uppercase tracking-wider opacity-70">Recorded · {result.minutes} min</span><br />{result.tier ? `${result.tier.slipMinutes} min rung: ${result.tier.consequence}` : 'Below every configured rung. No consequence applies.'}</p>}
    {slip.isError && <p data-testid="status-slip-error" className="mt-3 text-xs text-red-800">{apiError(slip.error)}</p>}
  </section>;
}
...
function SettingsPage() {
  ...
  const testMessage = useSendTestMessage();
  const qc = useQueryClient();
  const [tiers, setTiers] = useState<LadderTier[] | null>(null);
  ...
  const saveLadder = () => updateLadder.mutate({ data: { tiers: [...currentTiers].sort((a, b) => a.slipMinutes - b.slipMinutes) } }, { onSuccess: () => { setTiers(null); qc.invalidateQueries({ queryKey: getGetLadderQueryKey() }); } });
